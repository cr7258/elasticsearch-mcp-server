...
def test_cluster_parameter_routes_through_mcp(multi_cluster_mcp_server_in_kind):
    with port_forward("mcp-e2e-multi", 18020) as base_url:
        wait_for_http(f"{base_url}/healthz")
        mcp_url = f"{base_url}/mcp"

        primary_index = f"mcp-k8s-primary-{uuid.uuid4().hex[:8]}"
        secondary_index = f"mcp-k8s-secondary-{uuid.uuid4().hex[:8]}"
...
def test_omitted_cluster_uses_default_cluster_through_mcp(
    ...
    with port_forward("mcp-e2e-multi", 18021) as base_url:
        wait_for_http(f"{base_url}/healthz")
        mcp_url = f"{base_url}/mcp"

        index = f"mcp-k8s-default-{uuid.uuid4().hex[:8]}"
